...
def init_command(with_plasma=False):
    ...
    try:
        ...
        if with_plasma:
            print("✓ Plasma workspace has been built")
        else:
            print("ℹ Plasma workspace was skipped (use --with-plasma to build it)")
...
def install_session_command():
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print("You will need to run 'python ublue-kde-dev.py init --with-plasma' first.")
            sys.exit(1)
...
Examples:
  ...
  %(prog)s init --with-plasma  Initialize and build Plasma from master
  %(prog)s install-session     Install KDE session from development build
    ...
    init_parser.add_argument('--with-plasma', action='store_true',
                             help='Also build Plasma workspace after setup')
    ...
    if args.command == 'init':
        init_command(with_plasma=args.with_plasma)
